...
class Matching(Graph):
    ...
    def clone(self):
        ...
        for edge in self.edges:
            m.add_edge(m.find_vertex(edge.v.value), m.find_vertex(edge.w.value))
        return m
...
def main():
    ...
    for site in raw_material_sites:
        g.add_vertex(Vertex(site))

    for factory in factories:
        g.add_vertex(Vertex(factory))
    ...
    for transportation_company in transportation_companies:
        ...
        for i in range(int(transportation_company[0])):
            ...
            if current_state in raw_material_sites:
                ...
                if g.find_vertex(current_state) is None:
                    g.add_vertex(Vertex(current_state))

                current_other.append(current_state)

        for raw_material_site in current_raw_material_sites:
            for factory in current_factories:
                g.add_edge(g.find_vertex(raw_material_site), g.find_vertex(factory))

            for other in current_other:
                g.add_edge(g.find_vertex(raw_material_site), g.find_vertex(other))

        for factory in current_factories:
            for other in current_other:
                g.add_edge(g.find_vertex(other), g.find_vertex(factory))

    current_vertices = set([])
    vertex_to_remove = None

    for vertex in g.get_vertices():
        if vertex.get_value() not in raw_material_sites and vertex.get_value() not in factories:
            vertex_to_remove = vertex

            for edge in vertex.get_edges().values():
                current_vertices.add(edge.get_v())
                current_vertices.add(edge.get_w())

            g.remove_vertex(vertex)

    if vertex_to_remove is not None:
        current_vertices.remove(vertex_to_remove)

    for pair in combinations(current_vertices, r=2):
        g.add_edge(*pair)

    m = find_maximum_matching(g, Matching.from_graph(g))

    print(len(m.get_edges()))
